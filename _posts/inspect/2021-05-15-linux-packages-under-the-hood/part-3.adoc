...
To test our program, we need a basic package so that we can focus on the core logic of the APT installation process without having to support arcane use cases. We will use a new version of our package `hello` (the code is available in the link:https://github.com/julien-sobczak/linux-packages-under-the-hood[companion GitHub repository]):
