...
====
A `.deb` file is an `ar` archive. The `ar` command is an ancestor of the common `tar` command and was already present in the first Unix version in 1971! Now, this command is (mostly) only used by Debian packages. This archives contains 3 files:
