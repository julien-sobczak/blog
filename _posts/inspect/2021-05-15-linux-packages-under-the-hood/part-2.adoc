...
The entry point for the installation of a single package is the function `archivefiles`, and most specifically the function `process_archive`:
...
<2> The function `deferred_configure_conffile` renames the conffiles still ending with the suffix `.dpkg-new` created during the unpacking. This function is responsible to show the confirmation prompt.
