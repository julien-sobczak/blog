...
Fonts comes in different formats, like the OpenType format (`.otf`, `.otc`, `.ttf`, `.ttc`). A font is basically a collection of tables of glyphs. Here is a preview of the font `NotoSansArmenian.ttf`:

image::NotoSansArmenian.png[title="Examples of glypths present in the font _Noto Sans Armenian_ using FontGoggles."]

Most fonts (if not all) do not contains glyphs for every Unicode characters. For example, trying to display "Hello" using the same font found no glyphs and shows empty squares ☐:
...
Using Chrome DevTools, we can easily find out which font were used to render characters on screen:
...
*The challenge to render Unicode text is thus to convert a sequence of Unicode characters into a sequence of glyphs, using a list of available fonts*. This process is called _text shaping_. The logic is not simple. There is not always a 1-1 mapping between characters and glyphs. Unicode contains for example, combining characters, emojis characters, and some emojis like flags are made by combining 2 abstract characters (ex: the US flag 🇺🇸 is represented by the two characters `REGIONAL INDICATOR SYMBOL LETTER U` 🇺 + `REGIONAL INDICATOR SYMBOL LETTER S` 🇸). Implementing these rules is the job of the shaping engine.

Not all programs have to implement this logic from scratch. The library linK:https://www.freetype.org/[FreeType] is used by major systems (GNU/Linux, iOS, Android, Firefox, Ghostscript, etc.). This represents billion of devices!

Under the hood, this library depends on the shaping engine link:https://github.com/harfbuzz/harfbuzz[HarfBuzz], which does a lot of the hard work. Both programs are written in C and are low-level code. Here is a simple program using FreeType to render a sequence of Unicode characters using a single font (the original code appears in this link:https://gist.github.com/jokertarot/7583938[StackOverflow thread]):
...
The pistol has not always beeing a toy. If I rerun the same command using link:https://github.com/googlefonts/noto-emoji/blob/v2017-09-13-design-refresh/fonts/NotoColorEmoji.ttf[a previous version of the font]:
...
If you are curious about this text rendering, you can inspect the link:https://github.com/chromium/chromium/tree/93.0.4540.1/third_party/blink/renderer/platform/fonts[Chromium rendering engine] or read the link:https://harfbuzz.github.io/index.html:[documentation of the project HarfBuff].

==== Example: Slack Emojis

Slack and emojis are inseparable. Emojis makes long messages less boring and are indispensable to communicate about your mood, so that others can interpret your messages correctly. Slack even support custom emojis. How does Slack support so many different emojis? It's simple. They are not Unicode emojis.

Indeed, Unicode is an evolving standard but Slack cannot wait for Unicode to approve your custom emoji during the next quarter meeting. Slack cannot neither use the private-use range of characters as it would mean regenerating new font files with your emojis and make sure your recipient receives the new font too. So, Slack is using images even for standard Unicode characters for its web client (image:./slack-pistol-emoji.png[link="https://a.slack-edge.com/production-standard-emoji-assets/13.0/apple-large/1f52b@2x.png", height=18px] for the Pistol Emoji 🔫 U+1F52B).

Using images instead of Unicode characters is not problematic when the same application is used on both sides. When you are sending a message on Slack Desktop, you knows your recipient will check it using one of the clients provided by Slack (the web client, or the app for Windows/MacOS/Linux). Therefore, it is easy for Slack to make sure that the images for the emojis are available everywhere. The problem occurs when you try to copy/paste some text from Slack to a different application. In this case, what Slack is doing is replacing these images by textual representation like `:slightly_smiling_face:`:
...
Gmail is using the same approach. As a web application, Gmail cannot be totally sure of the fonts available on every device and to make sure the recipient sees the email like the sender viewed it, Gmail relies on images too.
...
YAT allows users to generate a unique chain of emoji characters to be used as their universal identifier on the Internet. Interopability is thus important. Your YAT name will be used on many other sites and YAT is thus restricted by the emojis defined by Unicode.
...
If you navigate on the YAT website, you will see a ton of beautiful emojis displayed in large definition. These emojis are not Unicode emojis, but hand-crafted illustrations in PNG format that you can also link:https://emojis.y.at/our-emojis#download[browse online].
