...
The command to install a Debian binary package file is `dpkg -i archive.deb`:
...
The command does a lot of interesting things and the C code is larger than the previous build command. We will split the code to focus on one concern at the time.

The link:https://man7.org/linux/man-pages/man1/dpkg.1.html[man page] details the installation steps and we will present the main code for every one of them. The entry point for the installation of a single package is the function `archivefiles`, and most specifically the function `process_archive`, which process a single Debian archive and which will be discussed at length in the different steps. Let's go!
...
archivefiles(const char *const *argv)
{
  ...
  for (i = 0; argv[i]; i++) {
    process_archive(argv[i]); // The subject of the next steps
  }
...
<2> Run the command `dpkg --control` to extract the `DEBIAN/` directory into a temporary directory.

Then, the code parses the `control` file to initialize the struct `pkginfo`, which is the main structure to represent a package. (You may check the const `fieldinfos` in `parse.c` to find the mapping between the file and the struct.) Here is a minimal version of this structure with the most important fields annotated:
...
----
<1> The function `parsedb` simply reads a file in link:https://man7.org/linux/man-pages/man5/deb822.5.html[Debian RFC822 control data format], the format we used to write the `control` file.
...
<2> Update the package status to keep trace the package has been partially installed. The status will be changed several times during the installation. The function `modstatdb_note` persists the new state to disk.
...
====
We still haven't talked about conffiles. The problem is simple. When upgrading a package, you want the package manager to overwrite the previous version of the files, except for configuration files. You don't want to lose your customizations, don’t you?
...
The package manager detects the conflict by keeping a checkum of the last installed version of every conffile (files named `md5sums` in the database) and asks the user what to do when it knows that the user has edited the file. Options exist to avoid the prompt and the default is, of course, to preserve existing conffile.
====

The unpacking starts by running the command `dpkg-deb --fsys-tarfile` to extract the content of `data.tar`. The command sends each file to a pipe created in the `process_archive` function and delegates to the function `tar_extractor` defined in `lib/dpkg/tarfn.c`, to then come back to the function `tarobject` defined in `archives.c`. All the rules presented in the previous comment are implemented in this last function and as you may imagine, the code is just too long to introduce it in this article.

The backup process consists mainly in extracting files with a special extension like `.dpkg-tmp`, `.dpkg-old` and `.dpkg-new`. Files are renamed to their definitive name if no problem occurs, except for conffiles, which must wait the last installation step to be renamed.
...
The execution code of the maintainer script `postrm` is similar to the previous scripts. What is more interesting is what happens at the end of the unpacking step. Indeed, the Dpkg database is updated to reflect the changes.
...
| `/var/lib/dpkg/status`
| A DEB822 file contains the status information for all the packages (i.e., the current state of each package and the fields in their `control` file).
...
We are getting close to the end of the function `process_archive`. The last instruction is `enqueue_package(pkg);`. This function is used to push a new package waiting to be configured. Since the `dpkg` command can be executed with several packages to install, the queue ensures all packages have been unpacked before proceeding to their final configuration.
...
What follows is a simplified.footnote:simplified[TODO] version of the `process_queue` function, but first the data structure:
...
Finally, the logic to empty the queue:
...
----
<1> The function `deferred_configure` is the function doing the configuration and is the subject of the next step.
...
The last step uses the same code as the command `dpkg --configure hello`, which may be used to reconfigure a package that had already been unpacked.

The configuration step is implemented by the function `deferred_configure` which focuses on a single package to configure. If the configuration cannot proceed, we have seen that the package will be enqueued to be reprocessed later or not. Here is a simplified version.footnote:simplified[]:
...
deferred_configure(struct pkginfo *pkg)
{
	...
	maintscript_postinst(pkg, "configure",	                     dpkg_version_is_informative(&pkg->configversion) ?
...
<2> The function `deferred_configure_conffile` renames the conffiles still ending with the suffix `.dpkg-new` created during the unpacking. Note that the function will remove the new file to preserve the user modifications after showing the confirmation prompt.
...
To review everything we discovered about the installation process, we will rewrite a basic version in Golang supporting only a fresh install. But first, let's remove the package or we will not be able to test our program:

[source,sh]
----
# cp -R /var/lib/dpkg /before # Backup the database to compare
...
# cp -R /var/lib/dpkg /after
# diff -r /before /after
diff -r /before/info/hello.list /after/info/hello.list
1,4d0
< /.
< /usr
< /usr/bin
< /usr/bin/hello
Only in /before/info: hello.md5sums
Only in /before/info: hello.postinst
Only in /before/info: hello.preinst
Only in /before/info: hello.prerm
diff -r /before/status /after/status
1431c1431
...
diff -r /before/status-old /after/status-old
1429a1430,1438
> Package: hello
> Status: install ok installed
> Priority: optional
...
vagrant$ sudo su
vagrant# /vagrant/dpkg --install /vagrant/hello.deb
...
root@bullseye:/home/vagrant# hello
hello world

root@bullseye:/home/vagrant# dpkg -s hello
Package: hello
Status: install ok installed
Priority: optional
Section: base
