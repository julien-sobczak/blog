...
This pistol has not always beeing a toy. If I rerun the same command using link:https://github.com/googlefonts/noto-emoji/blob/v2017-09-13-design-refresh/fonts/NotoColorEmoji.ttf[a previous version of the font]:
...
Gmail is using the same approach as Slack. As a web application, Gmail cannot be totally sure of the fonts available on every device and therefore, Gmail relies on images too so that the recipient is sure to see the email like the sender viewed it.
...
YAT allows you to generate a unique chain of emoji characters to be used as your universal identifier on the Internet. Interopability is thus important. Your YAT name will be used on many other sites and YAT is restricted by the emojis defined by Unicode.
