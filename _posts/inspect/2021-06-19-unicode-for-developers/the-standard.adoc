...
* Planes 15..16: the _Private Use Planes_. These code points can be freely used for any purpose, but their use requires that sender and receiver agree on their interpretation.
...
====
Unidoce encodings differ from many prior encodings that also use varied-length bytes but where overlap was permitted. For example:
...
====
Not breaking already encoded texts in UCS-2 (simply called Unicode at that time) was clearly the biggest challenge to extend the initial number of Unicode characters (2^16^ = 65536 characters).
...
UTF-8 is a variable-width like UTF-16, but offering compatibility with ASCII. That means Unicode code points U+0000..U+007F are converted to a single byte 0x00..0x7F in UTF-8 and are thus indistinguishable from ASCII itself. An ASCII document is thus a valid UTF-8 document (the reverse is rarely true).
...
In practice, UTF-8 has become the default Unicode encoding of the Web even if all three encodings are totally valid.
